...
function NavItems({ isModalView = false }){
    return(
        ...
        rounded-lg md:flex-row md:space-x-8 md:mt-0 md:border-0 bg-white ${isModalView ? "border-none" : "border border-gray-100" }`}>
            ...
            {
                isAdminView ? 
                adminNavOptions.map((item) => <li 
                className="cursor-pointer block py-2 pl-3 
                pr-4 text-gray-900 rounded md:p-0"
                key={item.id}>
                {item.label}
                </li>
                )
        ...
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
            ...
            <div className="flex md:order-2 gap-2 text-black">
                ...
                }
                {
                    ...
                    <button className={"mt-1.5 inline-block bg-black px-5 py-3 text-xs font-medium uppercase tracking-wide text-white"}>Log In</button>
                }
                <button
